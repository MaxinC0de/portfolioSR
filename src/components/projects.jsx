...
export default function Projects({ handleProjectSelect }) {
    ...
    const handleFocus = (e, title) => {
        setFocusedTitle(prev => prev === title ? title : title)
        handleProjectSelect(title)
    }
    ...
    return(
        <div className="text-[#fffef4] shadow-[5px_5px_0_0_#fff">
            ...
            {PROJECTS.map(({ id, title, description }, i) => (
                <>
                    ...
                    {!isDesktop && openProjectId === id && <ProjectDetails />} 
                </>
            ))}
            </ul>
        </div>
